...
architecture Behavioral of NOR_function is
...
signal data_in_array_a22 : WORD_ARRAY_type ((ninputs - 1) downto 0);  -- inputs of each Artificial Neuron
signal finished_a22 : STD_LOGIC := '0';  -- signals used for showing if the Artificial Neuron has finished/has to start the procedure
signal a22_output : STD_LOGIC_VECTOR (7 downto 0) := (others=>'0');   -- output of the Artificial Neuron
signal a22_sum_out : STD_LOGIC_VECTOR (7 downto 0) := (others=>'0');  -- LUT input of the Artificial Neuron
signal final_result_next, final_result_reg : STD_LOGIC_VECTOR (7 downto 0) := (others=>'0'); -- signals for registering the output of the selected neurons
signal middle_result_next, middle_result_reg : STD_LOGIC_VECTOR (7 downto 0) := (others=>'0'); -- signals for registering the input of the LUT of the selected neurons
...
-- adjustment of inputs of the Artificial Neuron
...
data_out <= a22_output(dec) OR a22_output(dec - 1); -- classification: if the output value is equals or larger than 0.5, it's classified to 1; else, it's classified to 0 
finish <= finished_a22; -- output of the finish signal
...
process(a22_sum_out, a22_output)
begin
...
begin
    if rising_edge(temporal) then
        ...
        aux6 <= '1';
        aux7 <= '1';        
        if (count_temporal rem a22_output'length = 0) then
            aux0 <= '0';
        elsif (count_temporal rem a22_output'length = 1) then
            aux1 <= '0';
        elsif (count_temporal rem a22_output'length = 2) then            
            aux2 <= '0';           
        elsif (count_temporal rem a22_output'length = 3) then            
            aux3 <= '0';      
        elsif (count_temporal rem a22_output'length = 4) then            
            aux4 <= '0';        
        elsif (count_temporal rem a22_output'length = 5) then            
            aux5 <= '0';       
        elsif (count_temporal rem a22_output'length = 6) then            
            aux6 <= '0';         
        elsif (count_temporal rem a22_output'length = 7) then            
            aux7 <= '0';         
        end if;
    end if;
end process;
