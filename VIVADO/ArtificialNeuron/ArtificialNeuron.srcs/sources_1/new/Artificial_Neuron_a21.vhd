...
entity Artificial_Neuron_a21 is
    ...
    Port ( data_in : in WORD_ARRAY_type ((ninputs - 1) downto 0);   -- signals of this array are the inputs of the neuron
           ...
           sum_out : out STD_LOGIC_VECTOR (7 downto 0);   -- signal that represents the input signal of the LUT of the selected neuron
...
attribute use_dsp48 of Artificial_Neuron_a21 : entity is "yes";
           
end Artificial_Neuron_a21;

architecture Behavioral of Artificial_Neuron_a21 is
...
signal start1_reg, start1_next, start2_reg, start2_next, start_def : STD_LOGIC := '0';  -- signals used for registering the start signal twice so the start signal used lasts one clock cycle
...
-- making the start signal last one clock cycle
start_def <= start1_reg AND (NOT start2_reg);
...
begin
    if reset = '1' then        
        ...
        start1_reg <= start1_next;
        start2_reg <= start2_next;
...
process(start, start1_reg, data_in, count_reg, weight_BRAM, address_reg, started_reg, product, sum)
begin 
    -- default values of the signals
    start1_next <= start;
    start2_next <= start1_reg;    
    ...
    if(start_def = '1') then    -- when start signal is 1, the procedure starts from the beginning
        started_next <= '1';
...
finish <= finished_reg; -- output of the finish signal
MAC_output <= s_reg;    -- output of the MAC
